...
| 6 | SDK publishing | **GitHub Packages npm registry** (`@abbasiwa/kindlepool-sdk` via npm.pkg.github.com) |
...
packages/sdk/               # @abbasiwa/kindlepool-sdk — TS wrappers over contract ABI + API
...
"@abbasiwa/kindlepool-sdk": "^0.1.0"  // via .npmrc scope → npm.pkg.github.com
```
...
1. Add `@stellar/stellar-sdk` + `@abbasiwa/kindlepool-sdk` to web
...
3. Vercel (kindlepool-web) — ✅ **LIVE** at **https://kindlepool.app** (account: mikwansa / team `abba15`, project `kindlepool`, deployment `kindlepool-i35k0rsde-abba15`). Old `cocor` account site + domain deleted.
4. Deploy contract to testnet → ✅ **v12 `CBLVO4HIYWJHV66YM4KMXNRBL6O4GMB6WNIWUT6M6MSWZLPWTMAYBLMX`** (initialized, flow 120/60/60)
5. Domain `kindlepool.app` — ✅ attached + verified on `abba15` project; serves the production deployment (HTTP 200)
6. Email: **Brevo SMTP** (replaces Resend) — connection verified

**Open blockers**: (a) Heroku account verification (user action); (b) Atlas Network Access allowlist for backend egress IP.
